  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <motion.div {...fadeUp(0.1)} className="lg:col-span-2 hud-border bg-card p-5 space-y-4 glow-primary">
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
              <FolderKanban className="h-4 w-4" /> Project Status
            </h3>
            <Link to="/projects" className="text-xs font-heading text-primary hover:underline flex items-center gap-1">
          ...
          <div className="space-y-3">
            {projects.map((p) => (
              <Link
                key={p.id}
                to={`/projects/${p.id}`}
                className="flex items-center justify-between p-3 rounded-md bg-background/50 hover:bg-background transition-colors group"
              >
                ...
                <span className={`text-xs font-display tracking-wider uppercase ml-3 flex-shrink-0 ${statusColor[p.status] || "text-muted-foreground"}`}>
                  {p.status}
                </span>
              </Link>
            ))}
        ...
        {/* News Feed */}
        <motion.div {...fadeUp(0.15)} className="hud-border bg-card p-5 space-y-4 glow-primary">
          <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
          ...
          <ul className="space-y-3">
            {news.map((n) => (
              <li key={n.id} className="flex items-start gap-3 text-sm">
                ...
                <div>
                  <p className="font-body text-foreground/80">{n.text}</p>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {/* Featured Characters */}
        <motion.div {...fadeUp(0.2)} className="hud-border bg-card p-5 space-y-4 glow-primary">
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
              <Users className="h-4 w-4" /> Key Personnel
            </h3>
            <Link to="/lore/characters" className="text-xs font-heading text-primary hover:underline flex items-center gap-1">
              View All <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
          <div className="mecha-line" />
            ...
            {characters.slice(0, 3).map((c) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-display tracking-wider text-muted-foreground uppercase">{c.race}</p>
                </div>
                {c.stats && (
        ...
        {/* Locations */}
        <motion.div {...fadeUp(0.25)} className="hud-border bg-card p-5 space-y-4 glow-primary">
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
              <MapPin className="h-4 w-4" /> Key Locations
            </h3>
            <Link to="/lore/places" className="text-xs font-heading text-primary hover:underline flex items-center gap-1">
              View All <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
          <div className="mecha-line" />
          <div className="space-y-3">
            {places.slice(0, 3).map((p) => (
              ...
              >
                <p className="text-sm font-heading text-foreground group-hover:text-primary transition-colors">{p.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{p.type} — {p.shortDesc.slice(0, 60)}…</p>
              </Link>
            ))}
          </div>
        </motion.div>
        ...
        {/* Technology */}
        <motion.div {...fadeUp(0.3)} className="hud-border bg-card p-5 space-y-4 glow-primary">
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
              <Cpu className="h-4 w-4" /> Technology
            </h3>
            <Link to="/lore/tech" className="text-xs font-heading text-primary hover:underline flex items-center gap-1">
              View All <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
          <div className="mecha-line" />
          <div className="space-y-3">
            {tech.slice(0, 3).map((t) => (
              <Link
                ...
                className="block p-2 rounded-md hover:bg-background/50 transition-colors group"
              >
                <p className="text-sm font-heading text-foreground group-hover:text-primary transition-colors">{t.name}</p>
      ...
      {/* Recent Gallery */}
      <motion.div {...fadeUp(0.35)} className="hud-border bg-card p-5 space-y-4 glow-primary">
        <div className="flex items-center justify-between">
          <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
            <Sparkles className="h-4 w-4" /> Recent Gallery
          </h3>
          <Link to="/gallery" className="text-xs font-heading text-primary hover:underline flex items-center gap-1">
            View All <ArrowRight className="h-3 w-3" />
          </Link>
        </div>
        <div className="mecha-line" />
      ...
      {/* Quick Actions */}
      <motion.div {...fadeUp(0.4)} className="hud-border bg-card p-5 space-y-4">
        <h3 className="font-heading text-sm tracking-[0.15em] text-accent-orange uppercase flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            >
              ...
              <div>
                <p className="text-sm font-heading text-foreground group-hover:text-primary transition-colors">{q.label}</p>
      ...
      {/* Footer status */}
      <motion.div {...fadeUp(0.45)} className="flex items-center justify-between text-xs text-muted-foreground font-heading tracking-wider px-1 pb-4">
